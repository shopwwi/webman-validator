...
namespace Shopwwi\WebmanValidator;

use Closure;
use Shopwwi\WebmanValidator\Helper\Str;
use Webman\File;
...
class Validator{
     ...
     * @return $this
     */
    public function remove($field, $rule = null): Validator
    {
        if (is_array($field)) {
     ...
     * @return $this
     */
    public function append($field, $rule = null): Validator
    {
        if (is_array($field)) {
            foreach ($field as $key => $rule) {
    ...
    public function checkRule($value, $rules): bool
    {
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof Closure) {
                ...
                // 判断验证类型
                [$type, $rule] = $this->getValidatorType($key, $rule);
    ...
    protected function checkItem(string $field, $value, $rules, array $data, string $title = '', array $msg = [])
    {
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof Closure) {
                ...
                // 判断验证类型
                [$type, $rule, $info] = $this->getValidatorType($key, $rule);
     ...
     * @return array
     */
    protected function getValidatorType($key, $rule): array
    {
        // 判断验证类型
        if (!is_numeric($key)) {
